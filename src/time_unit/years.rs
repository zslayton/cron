...
impl Years {
    ...
    pub(crate) fn contains_leap_year(&self) -> bool {
        match &self.spec {
            YearsSpec::All => true,
            YearsSpec::Ordinals(ordinals) => ordinals.iter().any(|year| is_leap_year(*year)),
            YearsSpec::Predicates {
                specifiers,
                wraparound_ranges,
            } => specifiers
                .iter()
                .any(|specifier| root_specifier_contains_leap_year(specifier, *wraparound_ranges)),
        }
    }
...
fn root_specifier_contains_leap_year(
    root_specifier: &RootSpecifier,
    wraparound_ranges: bool,
) -> bool {
    match root_specifier {
        RootSpecifier::Specifier(specifier) => {
            specifier_contains_leap_year(specifier, wraparound_ranges)
        }
        RootSpecifier::Period(specifier, step) if *step > 0 => {
            period_contains_leap_year(specifier, *step, wraparound_ranges)
        }
        RootSpecifier::Period(_, _)
        | RootSpecifier::NamedPoint(_)
        | RootSpecifier::LastDayOfMonth
...
fn specifier_contains_leap_year(specifier: &Specifier, wraparound_ranges: bool) -> bool {
    match specifier {
        Specifier::All => true,
        Specifier::Point(year) => *year <= LAST_YEAR && is_leap_year(*year),
        Specifier::Range(start, end) => {
            let (Ok(start), Ok(end)) = (ordinal_from_endpoint(start), ordinal_from_endpoint(end))
            else {
                return false;
            ...
            if start <= end {
                return range_contains_leap_year(start, end);
            }
            if !wraparound_ranges {
                return false;
            }
            range_contains_leap_year(start, LAST_YEAR) || range_contains_leap_year(FIRST_YEAR, end)
        }
    }
}

fn period_contains_leap_year(
    specifier: &Specifier,
    step: Ordinal,
    wraparound_ranges: bool,
) -> bool {
    match specifier {
        Specifier::All => stepped_sequence_contains_leap_year(FIRST_YEAR, LAST_YEAR, step),
        Specifier::Point(start) => stepped_sequence_contains_leap_year(*start, LAST_YEAR, step),
        Specifier::Range(start, end) => {
            let (Ok(start), Ok(end)) = (ordinal_from_endpoint(start), ordinal_from_endpoint(end))
            else {
                return false;
            ...
            if wraparound_ranges && start == end {
                return stepped_sequence_contains_leap_year(FIRST_YEAR, LAST_YEAR, step);
            }
            if start <= end {
                return stepped_sequence_contains_leap_year(start, end, step);
            }
            if !wraparound_ranges {
                return false;
            }

            stepped_sequence_contains_leap_year(start, LAST_YEAR, step)
                || leap_year_matching_wrapped_segment(start, end, step)
        }
    }
}

fn leap_year_matching_wrapped_segment(start: Ordinal, end: Ordinal, step: Ordinal) -> bool {
    let mut candidate = FIRST_YEAR;
    for _ in 0..100 {
        if candidate > end {
            return false;
        }
        if range_contains(candidate, start, end, step, true) {
            return true;
        }
        let Some(next) = candidate.checked_add(4) else {
            return false;
        };
        candidate = next;
    }
...
fn range_contains_leap_year(start: Ordinal, end: Ordinal) -> bool {
    if start > end {
        return false;
    }
    if end.saturating_sub(start) >= 399 {
        return true;
    }

    let remainder = start % 4;
    let mut candidate = start + if remainder == 0 { 0 } else { 4 - remainder };
    while candidate <= end {
        if is_leap_year(candidate) {
            return true;
        }
        let Some(next) = candidate.checked_add(4) else {
            return false;
...
fn stepped_sequence_contains_leap_year(start: Ordinal, end: Ordinal, step: Ordinal) -> bool {
    if step == 0 || start > end {
        return false;
    }
