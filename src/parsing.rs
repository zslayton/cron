...
fn last_named(i: &mut &str) -> PResult<SingleSpecifier> {
    delimited(
        multispace0,
...
fn single_point(i: &mut &str) -> PResult<SingleSpecifier> {
    digit1
        .try_map(u32::from_str)
...
fn single_named_point(i: &mut &str) -> PResult<SingleSpecifier> {
    alpha1
        .map(ToOwned::to_owned)
...
fn single_specifier(i: &mut &str) -> PResult<SingleSpecifier> {
    alt((single_point, single_named_point)).parse_next(i)
}

fn dom_last_point(i: &mut &str) -> PResult<RootSpecifier> {
    alt((
...
fn dow_last_point(i: &mut &str) -> PResult<RootSpecifier> {
    alt((
...
fn dow_root_specifier_with_any(i: &mut &str) -> PResult<RootSpecifier> {
    alt((
        period_with_any,
        dow_last_point,
        nth_of_month,
...
fn dom_root_specifier_with_any(i: &mut &str) -> PResult<RootSpecifier> {
    alt((
        period_with_any,
        weekday,
        dom_last_point,
...
fn dow_root_specifier_list_with_any(i: &mut &str) -> PResult<Vec<RootSpecifier>> {
    let list = separated(1.., dow_root_specifier_with_any, ",");
    let single_item = dow_root_specifier_with_any.map(|spec| vec![spec]);
    delimited(multispace0, alt((list, single_item)), multispace0).parse_next(i)
}

fn dom_root_specifier_list_with_any(i: &mut &str) -> PResult<Vec<RootSpecifier>> {
...
fn dow_field_with_any(i: &mut &str) -> PResult<Field> {
    let specifiers = dow_root_specifier_list_with_any.parse_next(i)?;
    Ok(Field { specifiers })
}

fn dom_field_with_any(i: &mut &str) -> PResult<Field> {
